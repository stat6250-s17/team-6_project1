...
;
proc means min q1 median q3 maxdata=homicide_analytic_file;
var victim_age;
run;
proc format;
...
table victim_sex*victim_age/norow nocol nopercent;
format victim_age victim_age_bin.;
run;
title;
footnote;
　
...
;
proc freq data=homicide_analytic_file;
table weapon/ noprint out=weapon_frequency;
run;
　
proc sort data=weapon_frequency out=weapon_sorted;
by descending count;
run;
proc print
noobs
...
run;
proc freq data=homicide_analytic_file;
table perpetrator_sex*perpetrator_age/nocol norow;
